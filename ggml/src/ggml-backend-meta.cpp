...
static void ggml_backend_meta_set_tensor_async(ggml_backend_t backend, ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    // A tensor that is split across multiple backends per segment (tensor
    // parallel, e.g. the KV cache across N GPUs) is copied with a 2D transfer
    // per backend, mirroring the synchronous set_tensor path above. the async
    // variants of the per-backend 2D copy land on each backend's compute
    // stream, so the caller still needs to synchronize the touched backends
            ...
            for (size_t s = 0; s < split_state.n_segments; s++) {
                for (size_t r = 0; r < split_state.nr[s]; r++) {
                    for (size_t j = 0; j < n_backends; j++) {
                        ...
                        ggml_backend_tensor_set_2d_async(ggml_backend_meta_simple_backend(backend, j), simple_tensor, (const char *) data + offset_data,
                            simple_offsets[j] + row_start * simple_tensor->nb[1], nbytes,
                            row_count, simple_tensor->nb[1], tensor->nb[1]);
                        offset_data       += nbytes;
                        simple_offsets[j] += nbytes;
...
static void ggml_backend_meta_get_tensor_async(ggml_backend_t backend, const ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    ...
    // A tensor that is split across multiple backends per segment (tensor
    // parallel, e.g. the KV cache across N GPUs) is copied with a 2D transfer
    // per backend, mirroring the synchronous get_tensor path above. the async
    // variants of the per-backend 2D copy land on each backend's compute
    // stream, so the caller still needs to synchronize the touched backends
    // before reading the host buffer.
    if (split_state.n_segments != 1 || split_state.nr[0] != 1) {
