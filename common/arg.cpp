...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"-cmp", "--checkpoint-min-prompt"}, "N",
        string_format("minimum prompt length in tokens before creating checkpoints (default: %d, 0 = no minimum)", params.checkpoint_min_prompt),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("checkpoint-min-prompt must be non-negative");
            }
            params.checkpoint_min_prompt = value;
        }
    ).set_env("LLAMA_ARG_CHECKPOINT_MIN_PROMPT").set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"-cram", "--cache-ram"}, "N",
