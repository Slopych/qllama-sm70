...
| `-cram, --cache-ram N` | set the maximum cache size in MiB (default: 8192, -1 - no limit, 0 - disable)[(more info)](https://github.com/ggml-org/llama.cpp/pull/16391)<br/>the cache collapses near-identical snapshots of the same conversation into one entry, so sampled tails do not accumulate and evict entries that are in use<br/>(env: LLAMA_ARG_CACHE_RAM) |
...
| `--slot-save-checkpoints N` | max number of context checkpoints to persist in each slot save, stored as a `<file>.ckpt` sidecar (default: 0, 0 = disabled)<br/>(env: LLAMA_ARG_SLOT_SAVE_CHECKPOINTS) |
